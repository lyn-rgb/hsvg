...
parser.add_argument('--data_root', default='data/BAIR', help='root directory for data')
...
parser.add_argument('--gpus', default='1', help='multiple gpus' )
...
parser.add_argument('--z_dim', type=int, default=16, help='dimensionality of z^p_t')
...
name = 'drsvg_I=vgg64x64-n_past=2-n_future=10-z^p_dim=16-z^c_dim=128-beta=0.000100-tc_lambda=0.001000-random_seqs=True'
...
for i in range(0, opt.N, opt.batch_size):
    ...
    ssim, psnr = make_gifs(train_x, i, 'train')
    ssim_train.append(ssim)
    psnr_train.append(psnr)

    # plot test
    test_x = next(testing_batch_generator)
    ssim, psnr = make_gifs(test_x, i, 'test')
